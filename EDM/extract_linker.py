...
def find_linker_atom_indices(acyclic_mol, macro_mol):
    ...
    print(f"[INFO] Acyclic heavy atoms: {acyclic_num_heavy_atoms}, Macro atoms: {macro_num_atoms}, Potential linker atoms: {macro_num_atoms - acyclic_num_heavy_atoms}", file=sys.stderr)
    
    if acyclic_num_heavy_atoms >= macro_num_atoms:
        print(f"[ERROR] Acyclic molecule has {acyclic_num_heavy_atoms} heavy atoms, which is >= macrocycle's {macro_num_atoms} atoms.", file=sys.stderr)
        return []
...
def add_second_attachment_point_and_return_indices(linker_mol):
    ...
    if not attachable_atoms:
        print("[ERROR] No atoms with available valence to attach new attachment point.")
        return None
...
def select_farthest_anchor_points(macro_mol, anchor_points):
    ...
    if len(non_ring_anchors) >= 2:
        ...
        print(f"[WARNING] Fewer than 2 non-ring anchor points ({len(non_ring_anchors)}), using ring atoms if necessary", file=sys.stderr)
    ...
    if farthest_pair is None:
        print(f"[WARNING] Could not find valid anchor point pair", file=sys.stderr)
        return None
...
def extract_linker_from_macro(macro_mol, linker_atom_indices):
    ...
    print("[DEBUG]",anchor_points)
    if anchor_points is None or len(anchor_points) != 2:
        print(f"[WARNING] Expected exactly 2 anchor points, found {len(anchor_points)} after selection", file=sys.stderr)

        # Try to fix: Add second dummy if only one present
        if anchor_points is not None and len(anchor_points) == 1:
            print("[INFO] Attempting to add second attachment point...")
            ...
            if new_anchor_points is None or len(new_anchor_points) != 2:
                print(f"[ERROR] Failed to recover a second anchor point automatically", file=sys.stderr)
                return None
            else:
                anchor_points.append(new_anchor_points[0])
    ...
    if selected_anchors is None or len(selected_anchors) != 2:
        print(f"[WARNING] Expected exactly 2 anchor points, found {len(selected_anchors)} after selection", file=sys.stderr)
        return None
    ...
    try:
        ...
        print(f"[WARNING] Could not sanitize linker or set coordinates: {e}", file=sys.stderr)
        return None
    return linker
...
def main():
    ...
    if acyclic_mol is None:
        print(f"[ERROR] Could not load acyclic file: {args.acyclic} -- exiting", file=sys.stderr)
        sys.exit(1)
    ...
    if not os.path.isfile(args.macro):
        print(f"[ERROR] Macrocycle SDF does not exist: {args.macro}", file=sys.stderr)
        sys.exit(1)
    
    macro_path = args.macro

    sdf_count += 1
    print(f"[INFO] Processing: {macro_path}", file=sys.stderr)
    macro_mol = load_mol(macro_path)
    if macro_mol is None:
        print(f"[WARNING] Could not load macrocycle file: {macro_path} -- skipping", file=sys.stderr)
        return

    # Extract linker
    linker_indices = find_linker_atom_indices(acyclic_mol, macro_mol)
    if not linker_indices:
        print(f"[WARNING] No linker atoms found for {macro_path}.", file=sys.stderr)
        return

    linker_mol = extract_linker_from_macro(macro_mol, linker_indices)
    if linker_mol is None:
        print(f"[WARNING] Failed to build valid linker from {macro_path}.", file=sys.stderr)
        return
    ...
    if dummy_count != 2:
        print(f"[WARNING] Linker from {macro_path} has {dummy_count} dummy atoms, expected 2 -- skipping", file=sys.stderr)
        return

    # Check for non-fragment: must be a single connected molecule
    fragments = Chem.GetMolFrags(linker_mol)
    if len(fragments) > 1:
        print(f"[WARNING] Linker from {macro_path} has {len(fragments)} fragments, expected 1 -- skipping", file=sys.stderr)
        return
    ...
    if linker_smiles and linker_smiles not in unique_smiles:
        ...
        linker_count += 1
        print(f"[OK] Linker extracted from {macro_path} and added to {args.output}")

    writer.close()
    print(f"[FINISHED] Processed {sdf_count} SDF files, extracted {linker_count} unique, valid, non-fragment linkers to {args.output}")
