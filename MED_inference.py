...
def run_macformer_on_smiles_in_memory(smiles, inter_dir):
    ...
    subprocess.run([
        "python", "GraphTransformer/predict.py",
        "--smiles", smiles,  
...
def clean_predicted_smiles(smiles):
    if "<UNK>" not in smiles:
        return smiles
    
    # Try replacing <UNK> with 'c' (aromatic carbon)
    candidate_c = smiles.replace("<UNK>", "c")
    if Chem.MolFromSmiles(candidate_c) is not None:
        return candidate_c
        
    # Try replacing <UNK> with 'C' (aliphatic carbon)
    candidate_C = smiles.replace("<UNK>", "C")
    if Chem.MolFromSmiles(candidate_C) is not None:
        return candidate_C
        
    # Try replacing <UNK> with '' (empty string)
    candidate_empty = smiles.replace("<UNK>", "")
    if Chem.MolFromSmiles(candidate_empty) is not None:
...
def filter_valid_smiles(smiles_list):
    ...
    for smile in smiles_list:
        ...
        smile = clean_predicted_smiles(smile)
        if smile.count("*") >= 2:
...
def load_mol(sdf_path):
    try:
        ...
        print(f"Warning: RDKit load error for {sdf_path}: {e}", file=sys.stderr)
    print(f"Warning: Could not load molecule from {sdf_path} (skipped)", file=sys.stderr)
    return None
...
def calculate_druglikeness_score(props, admet):
    ...
    if props['RotatableBonds'] < 10:
        score += 10
    if props['SA'] is not None and 1.0 <= props['SA'] <= 5.0:
        score += 30
    ...
    if admet['QED'] > 0.5:
        score += 30
    if admet['QED'] > 0.6:
        score += 20  # Additional bonus for exceptionally high drug-likeness QED in macrocycles
...
def save_macrocycle_grid(smiles_list, output_path, n_cols=5, mol_size=(280, 280)):
    """
    Render up to 25 valid macrocycle structures in a tidy grid image.
    Each cell is a clean 280x280 px tile; the overall figure scales proportionally.
    """
    ...
    valid_mols, valid_smis = [], []
    for smi in smiles_list:
        mol = Chem.MolFromSmiles(smi)
        if mol is not None:
            valid_mols.append(mol)
            valid_smis.append(smi)
        if len(valid_mols) >= 25:
            break

    if not valid_mols:
        print("[INFO] No valid macrocycles to draw in grid.")
        return

    n = len(valid_mols)
    n_rows = math.ceil(n / n_cols)
    labels = [f"MAC-{i+1}" for i in range(n)]
    ...
    # Save PIL image to a temporary file, then embed via matplotlib for a bordered frame
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
        tmp_path = tmp.name
    img.save(tmp_path)

    fig_w = n_cols * 2.8
    fig_h = n_rows * 2.8 + 0.6
    fig, ax = plt.subplots(figsize=(fig_w, fig_h))
    raw = mpimg.imread(tmp_path)
    ax.imshow(raw)
    ax.axis("off")
    ax.set_title(
        f"Generated Macrocycles — MED Pipeline  ({n} structures)",
    ...
    plt.close()
    os.unlink(tmp_path)
    print(f"[INFO] Saved macrocycle grid ({n} structures) → {output_path}")
...
def process_single_smiles(user_smiles, inter_dir, input_id):
    ...
    for i, aug_smi in enumerate(augmented):
        try:
            ...
            print(f"Warning: Macformer error on SMILES [{i}]: {e}")
                ...
                for frag in frags:
                    try:
                        ...
                        try:
                            ...
                            print(f"Warning: Skipping fragment: sanitization failed: {e}")
                    except Exception as e:
                        print(f"Warning: Skipping fragment: initial sanitization failed: {e}")
...
if __name__ == "__main__":
    ...
    if top_macrocycles:
        ...
        # ── Grid visualisation of all generated macrocycles ──────────────────
        all_macro_smiles = top_macrocycles_df["SMILES"].dropna().tolist()
        grid_out = os.path.join(INTER_DIR, "macrocycle_grid.png")
        save_macrocycle_grid(all_macro_smiles, grid_out)

        print(f"Top performing macrocycles for each input saved to {output_csv}")

        # --- ADDED: NEAT VISUALIZATIONS ---
        from rdkit.Chem import Draw
        import os
        vis_dir = os.path.join(INTER_DIR, "macrocycle_visuals")
        os.makedirs(vis_dir, exist_ok=True)
        for idx, row in top_macrocycles_df.iterrows():
            mol = Chem.MolFromSmiles(row['SMILES'])
            if mol:
                img_path = os.path.join(vis_dir, f"macrocycle_input_{idx+1}_score_{row.get('Druglikeness_Score', 0):.2f}.png")
                Draw.MolToFile(mol, img_path, size=(600, 600))
        print(f"Success: Saved beautiful 2D visualizations of the top macrocycles to {vis_dir}")
        # ----------------------------------
    else:
        print("Warning: No valid macrocycles found for any input SMILES")
